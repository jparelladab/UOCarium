...
public abstract class Animal extends Item implements Movable{
	...
	protected Animal(double xCoord, 
			double yCoord, 
			String spriteImage, 
			double length, 
			double height, 
			...
			int age, 
			double speed, 
			double requiredFoodQuantity, 
			double thresholdReverse, 
			int energy, 
	...
	private void setStatus(AnimalStatus status) {
		...
		//TODO: gestió de la imatge a mostrar en funció del status
		if (this.getStatus() == AnimalStatus.HEALTHY || this.getStatus() == AnimalStatus.SICK) {
			this.setSpriteImage(this.getSpriteImage().toString().split("_")[0].concat("_healthy.png"));
		} else {
			this.setSpriteImage(this.getSpriteImage().toString().split("_")[0].concat("_dead.png"));
		}
